"""LLM 降级调用测试 + 配置验证测试 + 动态权重合并测试 + 念头类型判断测试 + 日志清理测试 + Hindsight 重连测试"""
import pytest
import asyncio
import unittest.mock
from datetime import datetime, timedelta

from services.active_consciousness_service import call_llm_with_fallback, validate_active_consciousness_config
...
def test_validate_config_with_invalid_thresholds():
    """测试无效阈值验证 - send_threshold 必须大于 memory_threshold"""
    config = {
        "decision": {
            "send_threshold": 0.1,
            "memory_threshold": 0.6  # 大于 send_threshold
        }
    }
    errors = validate_active_consciousness_config(config)
    assert len(errors) > 0
    assert "发送阈值必须大于记忆阈值" in errors[0]
...
def test_validate_config_with_multiple_errors():
    ...
    config = {
        ...
        "decision": {
            "send_threshold": 0.2,
            "memory_threshold": 0.8  # 大于 send_threshold
        }
    }
    errors = validate_active_consciousness_config(config)
    assert len(errors) >= 2
